...
public class MovieInfoController implements MovieInfoApi {
    ...
    // Sinks.many().replay().all() — hot publisher that replays all past events to new subscribers
    ...
    public Mono<MovieInfoDocument> createMovieInfo(MovieInfoDocument movieInfoDocument) {
        return movieInfoService.createMovieInfo(movieInfoDocument)
                .flatMap(saved -> {
                    ...
                    // Save outbox event in the same reactive chain as the domain document save.
                    // flatMap ensures sequential execution; both operations share the same
                    // MongoDB reactive session context (though not a true 2-phase-commit).
    ...
    public Flux<MovieInfoDocument> getAllMovieInfos(Map<String, String> filterCriteria) {
        ...
        // .name("movie.info.get.all").metrics() instruments this Flux with Micrometer
    ...
    public Flux<MovieInfoDocument> getMovieInfoTailable() {
        return movieInfoService.getAllMovies(); // falls back to regular stream if not capped
    }
